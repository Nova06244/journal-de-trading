...
function buildSc(niv,val,dir,tpv,tpn,mode,biaisOverride,isStop){
  ...
  var e;
  if(isStop){
    // Buy Stop = au-dessus du niveau / Sell Stop = en dessous
...
function doAnalyse(price,pp,r1,r2,s1,s2,heure,mode,ctx,cassPullback,isRet,baseBiais){
  ...
  var all;
  if(bRET){
    // Retournement depuis LONG → SELL STOP sur R1/R2
    // Retournement depuis SHORT → BUY STOP sur S1/S2
    // Retournement seul → BUY+SELL STOP sur tous
    all=[
      buildSc("R2",vR2,"SHORT",vR1,"R1",mode,null,retFromLong||(!retFromShort)),
      buildSc("R1",vR1,"SHORT",vPP,"PP",mode,null,retFromLong||(!retFromShort)),
      buildSc("PP",vPP,dPP,dPP==="LONG"?vR1:vS1,dPP==="LONG"?"R1":"S1",mode,null,false),
      buildSc("S1",vS1,"LONG",vPP,"PP",mode,null,retFromShort||(!retFromLong)),
      buildSc("S2",vS2,"LONG",vS1,"S1",mode,null,retFromShort||(!retFromLong)),
    ];
  } else {
    all=[
  ...
  if(mode==="limite"){
    ...
    var isRetMode=biaisH4==="RETOURNEMENT"||biaisH4==="RETOURNEMENT_FROM_LONG"||biaisH4==="RETOURNEMENT_FROM_SHORT"||isRet;
    var retFromLong=biaisH4==="RETOURNEMENT_FROM_LONG"||(isRet&&baseBiais==="LONG");
    var retFromShort=biaisH4==="RETOURNEMENT_FROM_SHORT"||(isRet&&baseBiais==="SHORT");
  ...
  return(
    <div>
      ...
      <div style={{marginBottom:12}}>
        <div style={{fontSize:9,color:"#475569",marginBottom:5,letterSpacing:1}}>PRIX ACTUEL</div>
        <input type="number" step="0.00001" placeholder="1.XXXXX" value={props.price} onChange={function(e){props.setPrice(e.target.value);}} style={{width:"100%",background:"rgba(255,255,255,0.06)",border:"1px solid rgba(255,255,255,0.15)",borderRadius:8,padding:"9px 12px",color:"#fff",fontSize:13,fontFamily:"'DM Mono',monospace",outline:"none"}}/>
      </div>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:8}}>
          <div style={{background:"rgba(255,255,255,0.03)",border:"1px solid rgba(255,255,255,0.06)",borderRadius:10,padding:10}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:6,marginBottom:6}}>
              <button onClick={function(){props.setBiaisM30sel("");props.setIsRetournement(false);}} style={{padding:"7px 4px",background:props.biaisM30sel===""?"rgba(255,255,255,0.1)":"rgba(255,255,255,0.03)",border:"1px solid rgba(255,255,255,0.1)",borderRadius:7,color:"#64748b",fontSize:10,cursor:"pointer",fontFamily:"'DM Mono',monospace"}}>—</button>
            ...
            {(function(){
              ...
              return(
                <button onClick={function(){props.setIsRetournement(!props.isRetournement);}} disabled={props.biaisM30sel===""} style={{width:"100%",padding:"7px 4px",background:props.isRetournement?retBg:"rgba(255,255,255,0.03)",border:"1px solid "+(props.isRetournement?retBd:"rgba(255,255,255,0.1)"),borderRadius:7,color:props.isRetournement?retCol:props.biaisM30sel===""?"#2d3748":"#475569",fontSize:10,cursor:props.biaisM30sel===""?"not-allowed":"pointer",fontWeight:props.isRetournement?700:400,fontFamily:"'DM Mono',monospace",opacity:props.biaisM30sel===""?0.4:1}}>
                  ↩ RETOURNEMENT {props.isRetournement&&props.biaisM30sel?(props.biaisM30sel==="LONG"?"→ SELL STOP R1/R2":"→ BUY STOP S1/S2"):""}
                </button>
              );
            })()}
          </div>
      ...
      {!props.activePair||!props.newsStatut?null:props.newsStatut==="ROUGE"?(
        <div style={{background:"rgba(239,68,68,0.1)",border:"1px solid rgba(239,68,68,0.3)",borderRadius:10,padding:14,marginBottom:16,textAlign:"center"}}>
        ...
        <div style={{background:"rgba(34,197,94,0.08)",border:"1px solid rgba(34,197,94,0.3)",borderRadius:10,padding:14,marginBottom:16}}>
          ...
          <div style={{marginTop:10,display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8}}>
            {[
              {l:"SL",v:props.activePair==="EURUSD"?"10 pips":"13 pips",c:"#ef4444"},
              {l:"TP",v:props.activePair==="EURUSD"?"20 pips":"26 pips",c:"#22c55e"},
              {l:"RR",v:"1 : 2",c:"#93c5fd"}
          ...
          <button onClick={props.onAnalyse} style={{width:"100%",padding:11,background:"#22c55e",border:"none",borderRadius:8,color:"#000",fontSize:11,letterSpacing:2,fontWeight:700,cursor:"pointer",fontFamily:"'DM Mono',monospace",marginTop:10}}>
            → ANALYSER LE SETUP
          </button>
        </div>
      ):(
        <div style={{background:"rgba(234,179,8,0.08)",border:"1px solid rgba(234,179,8,0.3)",borderRadius:10,padding:14,marginBottom:16}}>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
            <div style={{display:"flex",alignItems:"center",gap:8}}>
              <span style={{fontSize:16}}>🟡</span>
          ...
          <div style={{marginTop:10,display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8}}>
            {[
              {l:"SL",v:props.activePair==="EURUSD"?"10 pips":"13 pips",c:"#ef4444"},
              {l:"TP",v:props.activePair==="EURUSD"?"15 pips":"18 pips",c:"#eab308"},
              {l:"RR",v:props.activePair==="EURUSD"?"1 : 1.5":"1 : 1.4",c:"#93c5fd"}
            ].map(function(x){return(
              <div key={x.l} style={{background:"rgba(0,0,0,0.2)",borderRadius:7,padding:"8px 4px",textAlign:"center"}}>
                <div style={{fontSize:9,color:"#475569",marginBottom:3}}>{x.l}</div>
                <div style={{fontSize:12,fontWeight:700,color:x.c}}>{x.v}</div>
          ...
          <button onClick={props.onAnalyse} style={{width:"100%",padding:11,background:"#eab308",border:"none",borderRadius:8,color:"#000",fontSize:11,letterSpacing:2,fontWeight:700,cursor:"pointer",fontFamily:"'DM Mono',monospace",marginTop:10}}>
            → ANALYSER LE SETUP
          </button>
        </div>
      )}
          ...
          <div style={{background:sigB,border:"1px solid "+sigC+"44",borderRadius:12,padding:16,marginBottom:14}}>
            ...
            <div style={{display:"flex",gap:8,marginBottom:12,flexWrap:"wrap"}}>
              ...
              <span style={{background:"rgba(0,0,0,0.2)",borderRadius:6,padding:"5px 10px",fontSize:10,color:res.fen?"#22c55e":"#f97316"}}>{res.fen?"✓ Fenêtre valide":"⚠ Hors fenêtre"}</span>
                  {res.biaisH4&&<span style={{background:res.biaisH4==="LONG"?"rgba(34,197,94,0.1)":(res.biaisH4&&res.biaisH4.includes("RETOURNEMENT"))?"rgba(168,85,247,0.1)":"rgba(239,68,68,0.1)",border:"1px solid "+(res.biaisH4==="LONG"?"rgba(34,197,94,0.3)":(res.biaisH4&&res.biaisH4.includes("RETOURNEMENT"))?"rgba(168,85,247,0.3)":"rgba(239,68,68,0.3)"),borderRadius:6,padding:"5px 10px",fontSize:10,color:res.biaisH4==="LONG"?"#22c55e":(res.biaisH4&&res.biaisH4.includes("RETOURNEMENT"))?"#c084fc":"#ef4444",fontWeight:600}}>{res.biaisH4==="LONG"?"▲ LONG":res.biaisH4==="SHORT"?"▼ SHORT":res.biaisH4==="RETOURNEMENT_FROM_LONG"?"▲+↩ SELL STOP":res.biaisH4==="RETOURNEMENT_FROM_SHORT"?"▼+↩ BUY STOP":"↩ RETOURN."}</span>}
    ...
    <div>
      <div style={CARD}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:12}}>
          {["EURUSD","GBPUSD"].map(function(p){
            return(
              <button key={p} onClick={function(){props.setTradePair(p);}} style={{padding:"8px",background:props.tradePair===p?"rgba(59,130,246,0.15)":"rgba(255,255,255,0.03)",border:"1px solid "+(props.tradePair===p?"rgba(59,130,246,0.5)":"rgba(255,255,255,0.06)"),borderRadius:8,color:props.tradePair===p?"#93c5fd":"#475569",fontSize:11,fontWeight:props.tradePair===p?700:400,cursor:"pointer",fontFamily:"'DM Mono',monospace"}}>
                {p==="EURUSD"?"EUR/USD":"GBP/USD"}
              </button>
            );
          })}
...
export default function PivotAgent(){
  ...
  async function onAnalyse(){
    ...
    setErr("");setRes(doAnalyse(price||"0",pp,r1,r2,s1,s2,heure,mode,ctxFull,cassPullback||(isRetournement&&biaisM30sel!==""),isRetournement,biaisM30sel));
  ...
  async function onAdd(){
    ...
    var data={date:jDate,heure:jHeure2,niveau:jNiv,dir:jDir,resultat:jRes,note:jNote,lot:lotTrade,gain:jGain!==""?parseFloat(jGain):null,commission:jComm!==""?parseFloat(jComm):0,pair:tradePair||"EURUSD"};
    setLoading(true);
    try {
      if(editTrade){
      ...
      setGapWeekend("");setCassPullback(false);setSignalType("");setSignalNote("");setIsRetournement(false);
