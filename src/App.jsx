...
function TabNews(props){
  ...
  async function fetchNews(){
    ...
    var dayName=new Date().toLocaleDateString("fr-FR",{weekday:"long"});
    var parts=["Tu es expert calendrier economique forex EUR/USD.",
      "Aujourd'hui "+today+" ("+dayName+").",
      "Fenetres heure Paris: F1=08h30-12h00 Londres, F2=13h30-15h30 New York.",
      "Analyse les news du jour impactant EUR/USD.",
      "ROUGE=NE PAS TRADER, ORANGE=PRUDENCE, VERT=TRADER.",
      "Reponds UNIQUEMENT en JSON: date, resume_macro, fenetre1(statut/verdict/horaire/news/conseil), fenetre2(statut/verdict/horaire/news/conseil), alertes_hors_fenetres, bce_fed(prochaine_bce/prochaine_fed/info).",
      "news contient: heure, titre, impact, detail.",
      "Pas de markdown, JSON pur uniquement."];
    ...
    try{
      ...
      if(!m)throw new Error("JSON non trouve");
      setNewsData(JSON.parse(m[0]));
