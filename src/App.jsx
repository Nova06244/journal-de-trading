...
function doAnalyse(price,pp,r1,r2,s1,s2,heure,mode,ctx,cassPullback){
  ...
  // Niveau du signal = premier niveau valide dans le sens du biais
  var sigNiv=cn;
    ...
    <div>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:8}}>
          ...
          <div style={{background:"rgba(255,255,255,0.03)",border:"1px solid rgba(255,255,255,0.06)",borderRadius:10,padding:10}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:6}}>
              <button onClick={function(){props.setCassPullback(false);}} style={{padding:"7px 4px",background:!props.cassPullback?"rgba(59,130,246,0.2)":"rgba(255,255,255,0.03)",border:"1px solid "+(!props.cassPullback?"rgba(59,130,246,0.5)":"rgba(255,255,255,0.1)"),borderRadius:7,color:!props.cassPullback?"#93c5fd":"#475569",fontSize:10,cursor:"pointer",fontWeight:!props.cassPullback?700:400,fontFamily:"'DM Mono',monospace"}}>◆ REJET PIVOT</button>
              <button onClick={function(){props.setCassPullback(true);}} style={{padding:"7px 4px",background:props.cassPullback?"rgba(234,179,8,0.2)":"rgba(255,255,255,0.03)",border:"1px solid "+(props.cassPullback?"rgba(234,179,8,0.5)":"rgba(255,255,255,0.1)"),borderRadius:7,color:props.cassPullback?"#eab308":"#475569",fontSize:10,cursor:"pointer",fontWeight:props.cassPullback?700:400,fontFamily:"'DM Mono',monospace"}}>⤴ CASS. PULLBACK</button>
            </div>
          </div>
        </div>
      </div>
      ...
      {res&&(
        <div>
          ...
          {(function(){
            // SETUP PRINCIPAL = toujours le niveau du résumé (sigNiv)
            var main=res.all.find(function(s){return s.niv===res.sigNiv;});
            // ALTERNATIVE = premier niveau valide différent du principal, dans le sens du biais
            var alt=res.all.find(function(s){return s.ok&&s.niv!==res.sigNiv;});
            var cards=[];
            if(main)cards.push(
              <div key={main.niv}>
                <p style={{fontSize:9,color:"#475569",letterSpacing:2,marginBottom:6}}>SETUP PRINCIPAL</p>
                <SCard sc={main} cur={true} mode={props.mode} lot={props.lot}/>
              </div>
            );
            if(alt)cards.push(
              <div key={alt.niv}>
                <p style={{fontSize:9,color:"#334155",letterSpacing:2,marginBottom:6}}>ALTERNATIVE</p>
                <SCard sc={alt} cur={false} mode={props.mode} lot={props.lot}/>
              </div>
            );
            return cards;
          })()}
...
export default function PivotAgent(){
  ...
  function onAnalyse(){
    ...
    var ctxFull=(biaisM30sel?("M30 "+biaisM30sel):"")+(ppM30val?" PPM30:"+ppM30val:"")+(gapWeekend?" gap:"+gapWeekend:"")+(cassPullback?" signal:cassure-pullback":"")+(ctx?" "+ctx:"");
    setErr("");setRes(doAnalyse(price||"0",pp,r1,r2,s1,s2,heure,mode,ctxFull,cassPullback));
  }
  ...
  async function onAdd(){
    ...
    try {
      ...
      setGapWeekend("");setCassPullback(false);
