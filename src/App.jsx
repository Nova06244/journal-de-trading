    ...
    <div>
      ...
      {res&&(
        <div>
          ...
          {(function(){
            var valid=res.all.filter(function(s){return s.ok;});
            var main=valid.find(function(s){return s.niv===res.sigNiv;})||valid[0];
            var alt=valid.find(function(s){return s.niv!==( main?main.niv:"");});
            var cards=[];
            if(main)cards.push(<div key={main.niv}><p style={{fontSize:9,color:"#475569",letterSpacing:2,marginBottom:6}}>SETUP PRINCIPAL</p><SCard sc={main} cur={true} mode={props.mode} lot={props.lot}/></div>);
            if(alt)cards.push(<div key={alt.niv}><p style={{fontSize:9,color:"#334155",letterSpacing:2,marginBottom:6}}>ALTERNATIVE</p><SCard sc={alt} cur={false} mode={props.mode} lot={props.lot}/></div>);
            return cards;
          })()}
        </div>
      )}
