...
function newsEffectif(news) {
  return news === "ROUGE+30" ? "ORANGE" : news;
}
...
function tradePL(t){
  ...
  if(t.resultat==="WIN") return TPP*10000*l*10+comm;
  return -(SLP*10000*l*10)+comm;
}
    ...
    <div>
      ...
      <div style={{background:"rgba(255,255,255,0.03)",border:"1px solid rgba(255,255,255,0.06)",borderRadius:10,padding:10,marginBottom:14}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr 1fr",gap:6}}>
          {[{v:"VERT",c:"#22c55e",bg:"rgba(34,197,94,0.2)",label:"🟢 VERT"},{v:"ORANGE",c:"#eab308",bg:"rgba(234,179,8,0.2)",label:"🟡 ORANGE"},{v:"ROUGE",c:"#ef4444",bg:"rgba(239,68,68,0.2)",label:"🔴 ROUGE"},{v:"ROUGE+30",c:"#f97316",bg:"rgba(249,115,22,0.2)",label:"🟠 +30MIN"}].map(function(s){
            return(
      ...
      {props.activePair&&props.newsStatut&&(
        props.newsStatut==="ROUGE"?(
          <div style={{background:"rgba(239,68,68,0.1)",border:"1px solid rgba(239,68,68,0.3)",borderRadius:10,padding:14,marginBottom:14,textAlign:"center"}}>
            ...
            <div style={{fontSize:10,color:"#fca5a5",marginTop:4}}>Attends 30 minutes après la news.</div>
          </div>
        ):props.newsStatut==="ROUGE+30"?(
          <div style={{background:"rgba(249,115,22,0.08)",border:"1px solid rgba(249,115,22,0.3)",borderRadius:10,padding:14,marginBottom:14}}>
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:10}}>
              <div style={{display:"flex",alignItems:"center",gap:8}}>
                <span style={{fontSize:16}}>🟠</span>
                <span style={{fontSize:11,fontWeight:700,color:"#f97316",letterSpacing:1}}>ROUGE +30MIN — PRUDENCE</span>
              </div>
              <span style={{fontSize:10,color:"#475569"}}>{props.activePair==="EURUSD"?"EUR/USD":"GBP/USD"}</span>
            </div>
            <div style={{fontSize:10,color:"#f97316",marginBottom:10}}>30 minutes écoulées — paramètres ORANGE appliqués</div>
      ...
      <div style={{marginTop:8,background:"rgba(255,255,255,0.02)",border:"1px solid rgba(255,255,255,0.08)",borderRadius:12,padding:14}}>
        ...
        {(props.activePair||props.newsStatut||props.biaisM30sel||props.biaisM15sel)&&(
          <div style={{background:"rgba(0,0,0,0.2)",borderRadius:8,padding:"8px 10px",marginBottom:10,display:"flex",flexWrap:"wrap",gap:6}}>
            ...
            {props.newsStatut&&<span style={{fontSize:9,color:props.newsStatut==="VERT"?"#22c55e":props.newsStatut==="ORANGE"?"#eab308":props.newsStatut==="ROUGE+30"?"#f97316":"#ef4444",background:props.newsStatut==="VERT"?"rgba(34,197,94,0.1)":props.newsStatut==="ORANGE"?"rgba(234,179,8,0.1)":props.newsStatut==="ROUGE+30"?"rgba(249,115,22,0.1)":"rgba(239,68,68,0.1)",borderRadius:4,padding:"2px 6px"}}>{props.newsStatut==="VERT"?"🟢":props.newsStatut==="ORANGE"?"🟡":props.newsStatut==="ROUGE+30"?"🟠":"🔴"} {props.newsStatut}</span>}
            ...
            {props.jTouches&&<span style={{fontSize:9,color:props.jTouches==="VIERGE"?"#fbbf24":"#f97316",background:props.jTouches==="VIERGE"?"rgba(251,191,36,0.1)":"rgba(249,115,22,0.1)",borderRadius:4,padding:"2px 6px"}}>{props.jTouches==="VIERGE"?"◉":"◈"} {props.jTouches}</span>}
            {props.jTypeTrade&&<span style={{fontSize:9,color:props.jTypeTrade==="REVERSAL"?"#c084fc":"#38bdf8",background:props.jTypeTrade==="REVERSAL"?"rgba(168,85,247,0.1)":"rgba(56,189,248,0.1)",borderRadius:4,padding:"2px 6px"}}>{props.jTypeTrade==="REVERSAL"?"↩":"→"} {props.jTypeTrade}</span>}
          </div>
        )}
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:10}}>
          <div style={{background:"rgba(255,255,255,0.03)",border:"1px solid rgba(255,255,255,0.06)",borderRadius:10,padding:10}}>
            <div style={{fontSize:9,color:"#475569",marginBottom:6,letterSpacing:1}}>TOUCHES</div>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:6}}>
              <button onClick={function(){props.setJTouches("");props.setJTypeTrade("");}} style={{padding:"7px 2px",background:props.jTouches===""?"rgba(255,255,255,0.1)":"rgba(255,255,255,0.03)",border:"1px solid rgba(255,255,255,0.1)",borderRadius:7,color:"#64748b",fontSize:10,cursor:"pointer",fontFamily:"'DM Mono',monospace"}}>—</button>
              <button onClick={function(){props.setJTouches("VIERGE");props.setJTypeTrade("REVERSAL");}} style={{padding:"7px 2px",background:props.jTouches==="VIERGE"?"rgba(251,191,36,0.2)":"rgba(255,255,255,0.03)",border:"1px solid "+(props.jTouches==="VIERGE"?"rgba(251,191,36,0.6)":"rgba(255,255,255,0.1)"),borderRadius:7,color:props.jTouches==="VIERGE"?"#fbbf24":"#475569",fontSize:9,cursor:"pointer",fontWeight:props.jTouches==="VIERGE"?700:400,fontFamily:"'DM Mono',monospace"}}>◉ VIERGE</button>
              <button onClick={function(){props.setJTouches("MULTI");props.setJTypeTrade("CONTINUATION");}} style={{padding:"7px 2px",background:props.jTouches==="MULTI"?"rgba(249,115,22,0.2)":"rgba(255,255,255,0.03)",border:"1px solid "+(props.jTouches==="MULTI"?"rgba(249,115,22,0.6)":"rgba(255,255,255,0.1)"),borderRadius:7,color:props.jTouches==="MULTI"?"#f97316":"#475569",fontSize:9,cursor:"pointer",fontWeight:props.jTouches==="MULTI"?700:400,fontFamily:"'DM Mono',monospace"}}>◈ MULTI</button>
            </div>
          </div>
          <div style={{background:"rgba(255,255,255,0.03)",border:"1px solid rgba(255,255,255,0.06)",borderRadius:10,padding:10}}>
            <div style={{fontSize:9,color:"#475569",marginBottom:6,letterSpacing:1}}>TYPE TRADE</div>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:6}}>
              <button onClick={function(){props.setJTypeTrade("");}} style={{padding:"7px 2px",background:props.jTypeTrade===""?"rgba(255,255,255,0.1)":"rgba(255,255,255,0.03)",border:"1px solid rgba(255,255,255,0.1)",borderRadius:7,color:"#64748b",fontSize:10,cursor:"pointer",fontFamily:"'DM Mono',monospace"}}>—</button>
              <button onClick={function(){props.setJTypeTrade("REVERSAL");}} style={{padding:"7px 2px",background:props.jTypeTrade==="REVERSAL"?"rgba(168,85,247,0.2)":"rgba(255,255,255,0.03)",border:"1px solid "+(props.jTypeTrade==="REVERSAL"?"rgba(168,85,247,0.6)":"rgba(255,255,255,0.1)"),borderRadius:7,color:props.jTypeTrade==="REVERSAL"?"#c084fc":"#475569",fontSize:9,cursor:"pointer",fontWeight:props.jTypeTrade==="REVERSAL"?700:400,fontFamily:"'DM Mono',monospace"}}>↩ RVRSL</button>
              <button onClick={function(){props.setJTypeTrade("CONTINUATION");}} style={{padding:"7px 2px",background:props.jTypeTrade==="CONTINUATION"?"rgba(56,189,248,0.2)":"rgba(255,255,255,0.03)",border:"1px solid "+(props.jTypeTrade==="CONTINUATION"?"rgba(56,189,248,0.6)":"rgba(255,255,255,0.1)"),borderRadius:7,color:props.jTypeTrade==="CONTINUATION"?"#38bdf8":"#475569",fontSize:9,cursor:"pointer",fontWeight:props.jTypeTrade==="CONTINUATION"?700:400,fontFamily:"'DM Mono',monospace"}}>→ CONT.</button>
            </div>
          </div>
        </div>
        ...
        <div>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
            ...
            <button onClick={function(){
              var headers=["date","heure","pair","niveau","timeframe","dir","resultat","gain","commission","lot","sl_pips","tp_pips","rr","news_statut","mm9","rsi7","signal_entree","touches","type_trade","date_sortie","heure_sortie","duree_minutes","note"];
...
function TabCal(props){
  ...
  var plMois=parseFloat(mDates.reduce(function(a,d){return a+dayPL(d);},0).toFixed(2));
  ...
  var beMois=mDates.reduce(function(a,d){return a+(byDay[d]||[]).filter(function(t){return t.resultat==="BE";}).length;},0);
  var totalWL=winMois+lossMois;
  var wrMois=totalWL?Math.round(winMois/totalWL*100):0;

  // Résumés par semaine (mois affiché)
  var weekMap={};
  mDates.forEach(function(d){
    var wk=getWeekKey(d);
    if(!weekMap[wk])weekMap[wk]={pl:0,wins:0,losses:0,be:0,trades:0};
    (byDay[d]||[]).forEach(function(t){
      weekMap[wk].pl+=tradePL(t);
      weekMap[wk].trades++;
      if(t.resultat==="WIN")weekMap[wk].wins++;
      else if(t.resultat==="LOSS")weekMap[wk].losses++;
      else weekMap[wk].be++;
    });
  });

  // Courbe capital (tous trades)
  var capDepart=props.capDepart?parseFloat(props.capDepart):200;
  var cumul=capDepart;
  var allSorted=[...props.journal].reverse();
  var points=allSorted.map(function(t,i){
    cumul+=tradePL(t);
    return{i,y:parseFloat(cumul.toFixed(2)),res:t.resultat};
  });
  var W=300,H=90;
  var minY=points.length?Math.min.apply(null,points.map(function(p){return p.y;}))-0.5:capDepart-1;
  var maxY=points.length?Math.max.apply(null,points.map(function(p){return p.y;}))+0.5:capDepart+1;
  function toY(v){return H-((v-minY)/(maxY-minY))*H;}
  var pathD=points.map(function(p,i){
    var x=(i/(Math.max(points.length-1,1)))*W;
    return(i===0?"M":"L")+x.toFixed(1)+","+toY(p.y).toFixed(1);
  }).join(" ");
  ...
  return(
    <div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr 1fr",gap:8,marginBottom:14}}>
        {[{l:"TRADES",v:String(trMois),c:"#e2e8f0"},{l:"P&L MOIS",v:(plMois>=0?"+":"")+plMois.toFixed(2)+"€",c:plMois>=0?"#22c55e":"#ef4444"},{l:"WIN RATE",v:totalWL?wrMois+"%":"—",c:wrMois>=60?"#22c55e":"#ef4444"},{l:"WIN/LOSS",v:winMois+"/"+lossMois,c:"#22c55e"}].map(function(x){
          return(
          ...
          {selTrades.map(function(t){
            ...
            return(
              <div key={t.id} style={{display:"flex",alignItems:"center",gap:8,background:"rgba(0,0,0,0.2)",borderRadius:8,padding:"8px 10px",marginBottom:6}}>
                <span style={{fontSize:10,fontWeight:600,color:t.resultat==="WIN"?"#22c55e":t.resultat==="LOSS"?"#ef4444":"#eab308"}}>{t.resultat==="WIN"?"✓":t.resultat==="LOSS"?"✗":"◎"}</span>
                <span style={{fontSize:9,color:"#94a3b8"}}>{t.heure}</span>
                <span style={{fontSize:9,color:LC[t.niveau]||"#64748b",fontWeight:600}}>{t.niveau}</span>
                <span style={{fontSize:9,color:t.dir==="LONG"?"#22c55e":"#ef4444"}}>{t.dir==="LONG"?"▲":"▼"}</span>
                <span style={{fontSize:9,color:"#475569",flex:1}}>{t.signal_entree||""}</span>
                {t.duree_minutes&&<span style={{fontSize:9,color:t.duree_minutes<=60?"#22c55e":t.duree_minutes<=120?"#eab308":"#ef4444"}}>⏱{formatDuree(t.duree_minutes)}</span>}
                <span style={{fontSize:11,fontWeight:700,color:pl>=0?"#22c55e":"#ef4444"}}>{pl>=0?"+":""}{pl.toFixed(2)}€</span>
              </div>
            );
          })}
      ...
      {Object.keys(weekMap).length>0&&(
        <div style={{marginBottom:4}}>
          <div style={{fontSize:9,color:"#475569",letterSpacing:2,marginBottom:8}}>SEMAINES — {MOIS[mo]} {yr}</div>
          {Object.entries(weekMap).map(function(entry){
            var wk=entry[0],data=entry[1];
            var pl=parseFloat(data.pl.toFixed(2));
            var wr=(data.wins+data.losses)?Math.round(data.wins/(data.wins+data.losses)*100):0;
            var d=new Date(wk),end=new Date(wk);
            end.setDate(end.getDate()+4);
            var label=d.getDate()+"/"+(d.getMonth()+1)+" → "+end.getDate()+"/"+(end.getMonth()+1);
            return(
              <div key={wk} style={{...CARD,borderLeft:"3px solid "+(pl>=0?"#22c55e":"#ef4444"),marginBottom:8}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
                  <div>
                    <div style={{fontSize:8,color:"#475569",letterSpacing:1,marginBottom:2}}>SEMAINE</div>
                    <div style={{fontSize:11,color:"#e2e8f0",fontWeight:600}}>{label}</div>
                  </div>
                  <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:22,color:pl>=0?"#22c55e":"#ef4444",letterSpacing:2}}>
                    {pl>=0?"+":""}{pl}€
                  </div>
                </div>
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr 1fr",gap:6}}>
                  {[{l:"TRADES",v:data.trades,c:"#e2e8f0"},{l:"WIN RATE",v:wr+"%",c:wr>=60?"#22c55e":"#ef4444"},{l:"WIN",v:data.wins,c:"#22c55e"},{l:"LOSS",v:data.losses,c:"#ef4444"}].map(function(k){
                    return(
                      <div key={k.l} style={{background:"rgba(0,0,0,0.25)",borderRadius:6,padding:"6px 4px",textAlign:"center"}}>
                        <div style={{fontSize:13,fontWeight:700,color:k.c}}>{k.v}</div>
                        <div style={{fontSize:8,color:"#475569"}}>{k.l}</div>
                      </div>
                    );
                  })}
                </div>
      ...
      {/* ── RÉSUMÉ MOIS ── */}
      {trMois>0&&(
        <div style={{...CARD,borderLeft:"3px solid "+(plMois>=0?"#22c55e":"#ef4444"),background:"rgba(59,130,246,0.04)"}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
            <div>
              <div style={{fontSize:8,color:"#475569",letterSpacing:1,marginBottom:2}}>TOTAL MOIS</div>
              <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:18,color:"#fff",letterSpacing:2}}>{MOIS[mo]} {yr}</div>
            </div>
            <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:28,color:plMois>=0?"#22c55e":"#ef4444",letterSpacing:2}}>
              {plMois>=0?"+":""}{plMois.toFixed(2)}€
            </div>
          </div>
          {/* Barres semaines */}
          {Object.keys(weekMap).length>0&&(
            <div style={{marginBottom:12}}>
              <div style={{fontSize:8,color:"#475569",marginBottom:6,letterSpacing:1}}>P&L PAR SEMAINE</div>
              <div style={{display:"flex",gap:6,alignItems:"flex-end",height:50}}>
                {Object.entries(weekMap).map(function(entry,idx){
                  var wk=entry[0],data=entry[1];
                  var pl=parseFloat(data.pl.toFixed(2));
                  var maxAbs=Math.max.apply(null,Object.values(weekMap).map(function(w){return Math.abs(parseFloat(w.pl.toFixed(2)));}))||1;
                  var h=Math.max(4,Math.round((Math.abs(pl)/maxAbs)*44));
                  return(
                    <div key={wk} style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",gap:3}}>
                      <div style={{fontSize:8,color:pl>=0?"#22c55e":"#ef4444",fontWeight:600}}>{pl>=0?"+":""}{pl.toFixed(1)}</div>
                      <div style={{width:"100%",height:h+"px",background:pl>=0?"rgba(34,197,94,0.5)":"rgba(239,68,68,0.5)",borderRadius:3}}/>
                      <div style={{fontSize:7,color:"#334155"}}>S{idx+1}</div>
                    </div>
                  );
                })}
              </div>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr 1fr 1fr",gap:6}}>
            {[{l:"TRADES",v:trMois,c:"#e2e8f0"},{l:"WIN RATE",v:totalWL?wrMois+"%":"—",c:wrMois>=60?"#22c55e":"#ef4444"},{l:"WIN",v:winMois,c:"#22c55e"},{l:"LOSS",v:lossMois,c:"#ef4444"},{l:"BE",v:beMois,c:"#eab308"}].map(function(k){
              return(
                <div key={k.l} style={{background:"rgba(0,0,0,0.25)",borderRadius:6,padding:"6px 2px",textAlign:"center"}}>
                  <div style={{fontSize:12,fontWeight:700,color:k.c}}>{k.v}</div>
                  <div style={{fontSize:7,color:"#475569"}}>{k.l}</div>
                </div>
              );
            })}
          </div>
      ...
      {points.length>0&&(
        <div style={{...CARD,marginTop:14}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
            <span style={{fontSize:9,color:"#475569",letterSpacing:2}}>COURBE DE CAPITAL</span>
            <span style={{fontSize:12,fontWeight:700,color:cumul>=capDepart?"#22c55e":"#ef4444"}}>
              {cumul.toFixed(2)}€ {cumul>=capDepart?"▲":"▼"} {cumul>=capDepart?"+":""}{(cumul-capDepart).toFixed(2)}€
            </span>
          </div>
          <svg width="100%" viewBox={"-8 -8 "+(W+16)+" "+(H+16)}>
            {[0,0.5,1].map(function(p,i){
              var y=toY(minY+(maxY-minY)*p);
              return <line key={i} x1={0} y1={y} x2={W} y2={y} stroke="rgba(255,255,255,0.04)" strokeWidth={1}/>;
            })}
            <line x1={0} y1={toY(capDepart)} x2={W} y2={toY(capDepart)} stroke="rgba(59,130,246,0.25)" strokeWidth={1} strokeDasharray="4,3"/>
            <defs>
            ...
            {points.length>1&&<path d={pathD+" L"+W+","+H+" L0,"+H+" Z"} fill="url(#gcap)"/>}
            {points.length>1&&<path d={pathD} fill="none" stroke="#22c55e" strokeWidth={2}/>}
            {points.map(function(p,i){
              var x=(i/(Math.max(points.length-1,1)))*W;
              var col=p.res==="WIN"?"#22c55e":p.res==="LOSS"?"#ef4444":"#eab308";
              return <circle key={i} cx={x} cy={toY(p.y)} r={3} fill={col} stroke="#0a0e1a" strokeWidth={1.5}/>;
            })}
          </svg>
          <div style={{display:"flex",gap:12,fontSize:8,color:"#475569",justifyContent:"center",marginTop:6}}>
            <span><span style={{color:"#22c55e"}}>●</span> WIN</span>
            <span><span style={{color:"#ef4444"}}>●</span> LOSS</span>
            <span><span style={{color:"#eab308"}}>●</span> BE</span>
            <span><span style={{color:"#3b82f6"}}>--</span> Départ</span>
          </div>
        </div>
      )}
    </div>
...
export default function PivotAgent(){
  ...
  var [jTouches,setJTouches]=useState("");
  var [jTypeTrade,setJTypeTrade]=useState("");
  ...
  async function onAdd(){
    ...
    var data={
      ...
      timeframe:jTf||null,
      touches:jTouches||null,
      type_trade:jTypeTrade||null,
    ...
    try {
      ...
      setJDateSortie(TODAY);setJHeureSortie("");setJTf("");setJTouches("");setJTypeTrade("");
    ...
    <div style={{minHeight:"100vh",background:BG,fontFamily:"'DM Mono','Courier New',monospace",color:"#e2e8f0",padding:"18px 14px",backgroundImage:"radial-gradient(ellipse at 20% 0%,rgba(59,130,246,.06) 0%,transparent 60%)"}}>
      ...
      <div style={{maxWidth:640,margin:"0 auto"}}>
        {tab==="signal"&&<TabSignal
          ...
          jTouches={jTouches} setJTouches={setJTouches}
          jTypeTrade={jTypeTrade} setJTypeTrade={setJTypeTrade}
          editTrade={editTrade}
        ...
        {tab==="cal"&&<TabCal journal={journal} capDepart={capDepart} capSaved={capSaved}/>}
