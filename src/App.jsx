    ...
    <div>
      <div style={CARD}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:10}}>
          <div>
            <div style={{fontSize:9,color:"#475569",marginBottom:4}}>DATE (AAAA-MM-JJ)</div>
            <input type="text" value={props.jDate} onChange={function(e){props.setJDate(e.target.value);}} placeholder="2026-05-03" style={{...INP,fontSize:12,padding:"9px 8px",letterSpacing:0}}/>
          </div>
          <div>
            <div style={{fontSize:9,color:"#475569",marginBottom:4}}>HEURE (HH:MM)</div>
            <input type="text" value={props.jHeure2} onChange={function(e){props.setJHeure2(e.target.value);}} placeholder="09:30" style={{...INP,fontSize:12,padding:"9px 8px",letterSpacing:0}}/>
          </div>
        </div>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8,marginBottom:10}}>
          <div>
...
function TabNews(props){
  var items=["Vérifier forexfactory.com ou investing.com ce matin","Spread ICMarkets normal (proche de 0) ?","Niveaux pivots sauvegardés pour aujourd'hui ?","Pas de décision BCE ou Fed ce jour ?"];

  // News classées par fenêtre
  var f1=[
    {h:"09:00",l:"CPI Zone Euro",i:"ROUGE",c:"Annuler les ordres Londres"},
    {h:"09:30",l:"PMI Manufacturier UK",i:"ORANGE",c:"Prudence en début de session"},
    {h:"10:00",l:"PMI Zone Euro / ISM",i:"ORANGE",c:"Surveiller la volatilité"},
    {h:"11:00",l:"Discours BCE (Lagarde)",i:"ROUGE",c:"Pas d'ordres ouverts"},
  ];
  var f2=[
    {h:"13:45",l:"Décision BCE (jeudi)",i:"ROUGE",c:"Fenêtre 2 annulée"},
    {h:"14:15",l:"ADP Emploi US (mercredi)",i:"ORANGE",c:"Réduire la taille de lot"},
    {h:"14:30",l:"NFP — 1er vendredi du mois",i:"ROUGE",c:"Ne pas trader ce jour"},
    {h:"14:30",l:"CPI USA / PIB USA",i:"ROUGE",c:"Fermer les ordres avant"},
    {h:"15:00",l:"ISM Services USA",i:"ORANGE",c:"Surveiller en fin de fenêtre"},
    {h:"15:45",l:"PMI USA (S&P Global)",i:"ORANGE",c:"Prudence en fin de session"},
  ];
  var horsF=[
    {h:"20:00",l:"Décision Fed (FOMC)",i:"ROUGE",c:"Impacte le lendemain matin"},
    {h:"20:30",l:"Conférence Powell",i:"ROUGE",c:"Forte volatilité nocturne"},
  ];

  // Statuts fenêtres (à cocher manuellement)
  var st1=props.checks[4]?"ROUGE":props.checks[5]?"ORANGE":"VERT";
  var st2=props.checks[6]?"ROUGE":props.checks[7]?"ORANGE":"VERT";
  ...
  return(
    <div>
      ...
      <div style={{background:"rgba(239,68,68,0.06)",border:"1px solid rgba(239,68,68,0.2)",borderRadius:12,padding:16,marginBottom:14}}>
        <p style={{fontSize:10,letterSpacing:2,color:"#ef4444",marginBottom:12,marginTop:0}}>⚠ CHECKLIST MATIN</p>
      ...
      {/* Fenêtre 1 */}
      <div style={{background:stBg(st1),border:"1px solid "+stBorder(st1),borderRadius:12,padding:16,marginBottom:14}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
          <div>
            <div style={{fontSize:9,color:"#475569",letterSpacing:2,marginBottom:4}}>FENÊTRE 1 — LONDRES</div>
            <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:20,color:"#fff",letterSpacing:2}}>08h30 → 12h00</div>
          </div>
          <div style={{background:stBg(st1),border:"1px solid "+stBorder(st1),borderRadius:8,padding:"6px 12px"}}>
            <span style={{fontSize:11,fontWeight:700,color:stCol(st1)}}>{stLabel(st1)}</span>
          </div>
        </div>
        <div style={{display:"flex",gap:8,marginBottom:12}}>
          <button onClick={function(){props.tc(4);}} style={{flex:1,padding:"6px",background:props.checks[4]?"rgba(239,68,68,0.2)":"rgba(255,255,255,0.04)",border:"1px solid "+(props.checks[4]?"rgba(239,68,68,0.4)":"rgba(255,255,255,0.1)"),borderRadius:6,color:props.checks[4]?"#ef4444":"#64748b",fontSize:10,cursor:"pointer",fontFamily:"'DM Mono',monospace"}}>🔴 NEWS ROUGE</button>
          <button onClick={function(){props.tc(5);}} style={{flex:1,padding:"6px",background:props.checks[5]?"rgba(234,179,8,0.2)":"rgba(255,255,255,0.04)",border:"1px solid "+(props.checks[5]?"rgba(234,179,8,0.4)":"rgba(255,255,255,0.1)"),borderRadius:6,color:props.checks[5]?"#eab308":"#64748b",fontSize:10,cursor:"pointer",fontFamily:"'DM Mono',monospace"}}>🟡 NEWS ORANGE</button>
          <button onClick={function(){if(props.checks[4])props.tc(4);if(props.checks[5])props.tc(5);}} style={{flex:1,padding:"6px",background:st1==="VERT"?"rgba(34,197,94,0.2)":"rgba(255,255,255,0.04)",border:"1px solid "+(st1==="VERT"?"rgba(34,197,94,0.4)":"rgba(255,255,255,0.1)"),borderRadius:6,color:st1==="VERT"?"#22c55e":"#64748b",fontSize:10,cursor:"pointer",fontFamily:"'DM Mono',monospace"}}>🟢 LIBRE</button>
        </div>
        <p style={{fontSize:10,color:"#475569",letterSpacing:1,marginBottom:8,marginTop:0}}>NEWS HABITUELLES SUR CETTE FENÊTRE</p>
        {f1.map(function(n,i){
          return(
            <div key={i} style={{display:"flex",gap:10,padding:"7px 0",borderBottom:"1px solid rgba(255,255,255,0.04)"}}>
              <span style={{fontSize:10,color:"#64748b",minWidth:38}}>{n.h}</span>
              <div style={{flex:1}}>
      ...
      {/* Fenêtre 2 */}
      <div style={{background:stBg(st2),border:"1px solid "+stBorder(st2),borderRadius:12,padding:16,marginBottom:14}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
          <div>
            <div style={{fontSize:9,color:"#475569",letterSpacing:2,marginBottom:4}}>FENÊTRE 2 — NEW YORK</div>
            <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:20,color:"#fff",letterSpacing:2}}>13h30 → 15h30</div>
          </div>
          <div style={{background:stBg(st2),border:"1px solid "+stBorder(st2),borderRadius:8,padding:"6px 12px"}}>
            <span style={{fontSize:11,fontWeight:700,color:stCol(st2)}}>{stLabel(st2)}</span>
          </div>
        </div>
        <div style={{display:"flex",gap:8,marginBottom:12}}>
          <button onClick={function(){props.tc(6);}} style={{flex:1,padding:"6px",background:props.checks[6]?"rgba(239,68,68,0.2)":"rgba(255,255,255,0.04)",border:"1px solid "+(props.checks[6]?"rgba(239,68,68,0.4)":"rgba(255,255,255,0.1)"),borderRadius:6,color:props.checks[6]?"#ef4444":"#64748b",fontSize:10,cursor:"pointer",fontFamily:"'DM Mono',monospace"}}>🔴 NEWS ROUGE</button>
          <button onClick={function(){props.tc(7);}} style={{flex:1,padding:"6px",background:props.checks[7]?"rgba(234,179,8,0.2)":"rgba(255,255,255,0.04)",border:"1px solid "+(props.checks[7]?"rgba(234,179,8,0.4)":"rgba(255,255,255,0.1)"),borderRadius:6,color:props.checks[7]?"#eab308":"#64748b",fontSize:10,cursor:"pointer",fontFamily:"'DM Mono',monospace"}}>🟡 NEWS ORANGE</button>
          <button onClick={function(){if(props.checks[6])props.tc(6);if(props.checks[7])props.tc(7);}} style={{flex:1,padding:"6px",background:st2==="VERT"?"rgba(34,197,94,0.2)":"rgba(255,255,255,0.04)",border:"1px solid "+(st2==="VERT"?"rgba(34,197,94,0.4)":"rgba(255,255,255,0.1)"),borderRadius:6,color:st2==="VERT"?"#22c55e":"#64748b",fontSize:10,cursor:"pointer",fontFamily:"'DM Mono',monospace"}}>🟢 LIBRE</button>
        </div>
        <p style={{fontSize:10,color:"#475569",letterSpacing:1,marginBottom:8,marginTop:0}}>NEWS HABITUELLES SUR CETTE FENÊTRE</p>
        {f2.map(function(n,i){
          return(
      ...
      {/* Hors fenêtres */}
      <div style={CARD}>
        <p style={{fontSize:10,letterSpacing:2,color:"#475569",marginBottom:12,marginTop:0}}>⚡ HORS FENÊTRES — À SURVEILLER</p>
        {horsF.map(function(n,i){
          return(
            <div key={i} style={{display:"flex",gap:10,padding:"7px 0",borderBottom:"1px solid rgba(255,255,255,0.04)"}}>
              <span style={{fontSize:10,color:"#64748b",minWidth:38}}>{n.h}</span>
              <div style={{flex:1}}>
                <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:2}}>
                  <span style={{fontSize:11,color:"#e2e8f0"}}>{n.l}</span>
                  <span style={{fontSize:9,background:"rgba(239,68,68,0.15)",color:"#ef4444",padding:"1px 6px",borderRadius:4}}>{n.i}</span>
                </div>
                <span style={{fontSize:10,color:"#475569"}}>{n.c}</span>
              </div>
            </div>
        ...
        <div style={{marginTop:12,background:"rgba(59,130,246,0.06)",borderRadius:8,padding:"10px 12px",fontSize:11,color:"#64748b"}}>
          📅 Vérifie chaque matin sur <span style={{color:"#3b82f6"}}>forexfactory.com</span>
        </div>
      </div>
    </div>
  );
...
export default function PivotAgent(){
  ...
  var [checks,setChecks]=useState([false,false,false,false,false,false,false,false]);
