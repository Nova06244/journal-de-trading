...
function doAnalyse(price,pp,r1,r2,s1,s2,heure,mode,ctx,cassPullback){
  ...
  var bRET=biaisH4==="RETOURNEMENT";
  var all;
  if(bRET){
    // Mode retournement : Buy/Sell STOP sur cassure du niveau
    all=[
      buildSc("R2",vR2,"SHORT",vR1,"R1",mode,null,true),
      buildSc("R1",vR1,"SHORT",vPP,"PP",mode,null,true),
      buildSc("PP",vPP,dPP,dPP==="LONG"?vR1:vS1,dPP==="LONG"?"R1":"S1",mode,null,false),
      buildSc("S1",vS1,"LONG",vPP,"PP",mode,null,true),
      buildSc("S2",vS2,"LONG",vS1,"S1",mode,null,true),
    ];
  } else {
    all=[
  ...
  if(mode==="limite"){
    ...
    var isRetMode=biaisH4==="RETOURNEMENT";
    ...
    <div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:14}}>
        {["EURUSD","GBPUSD"].map(function(p){
          return(
            <button key={p} onClick={function(){props.setActivePair(p);}} style={{padding:"10px 8px",background:props.activePair===p?"rgba(59,130,246,0.15)":"rgba(255,255,255,0.03)",border:"1px solid "+(props.activePair===p?"rgba(59,130,246,0.5)":"rgba(255,255,255,0.06)"),borderRadius:10,color:props.activePair===p?"#93c5fd":"#475569",fontSize:11,fontWeight:props.activePair===p?700:400,cursor:"pointer",fontFamily:"'DM Mono',monospace",letterSpacing:1}}>
      ...
      <div style={{background:"rgba(59,130,246,0.08)",border:"1px solid rgba(59,130,246,0.2)",borderRadius:10,padding:"10px 14px",marginBottom:14}}>
        <div style={{fontSize:10,fontWeight:600,color:"#93c5fd",marginBottom:2}}>📊 CONFIRMATION M15 — {props.activePair==="EURUSD"?"EUR/USD":"GBP/USD"}</div>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:8}}>
          <div style={{background:"rgba(255,255,255,0.03)",border:"1px solid rgba(255,255,255,0.06)",borderRadius:10,padding:10}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr 1fr 1fr",gap:5}}>
              ...
              <button onClick={function(){props.setBiaisM30sel("RETOURNEMENT");}} style={{padding:"7px 4px",background:props.biaisM30sel==="RETOURNEMENT"?"rgba(168,85,247,0.2)":"rgba(255,255,255,0.03)",border:"1px solid "+(props.biaisM30sel==="RETOURNEMENT"?"rgba(168,85,247,0.5)":"rgba(255,255,255,0.1)"),borderRadius:7,color:props.biaisM30sel==="RETOURNEMENT"?"#c084fc":"#475569",fontSize:10,cursor:"pointer",fontWeight:props.biaisM30sel==="RETOURNEMENT"?700:400,fontFamily:"'DM Mono',monospace"}}>↩ RETOURN.</button>
            </div>
          </div>
          <div style={{background:"rgba(255,255,255,0.03)",border:"1px solid rgba(255,255,255,0.06)",borderRadius:10,padding:10}}>
      ...
      <div style={{background:"rgba(255,255,255,0.03)",border:"1px solid rgba(255,255,255,0.06)",borderRadius:10,padding:10,marginBottom:12}}>
        <div style={{fontSize:9,color:"#475569",marginBottom:6,letterSpacing:1}}>STATUT NEWS DU JOUR</div>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:6}}>
          ...
          <div style={{background:sigB,border:"1px solid "+sigC+"44",borderRadius:12,padding:16,marginBottom:14}}>
            ...
            <div style={{display:"flex",gap:8,marginBottom:12,flexWrap:"wrap"}}>
              ...
              <span style={{background:"rgba(0,0,0,0.2)",borderRadius:6,padding:"5px 10px",fontSize:10,color:res.fen?"#22c55e":"#f97316"}}>{res.fen?"✓ Fenêtre valide":"⚠ Hors fenêtre"}</span>
                  {res.biaisH4&&<span style={{background:res.biaisH4==="LONG"?"rgba(34,197,94,0.1)":res.biaisH4==="CONTRA"?"rgba(234,179,8,0.1)":res.biaisH4==="RETOURNEMENT"?"rgba(168,85,247,0.1)":"rgba(239,68,68,0.1)",border:"1px solid "+(res.biaisH4==="LONG"?"rgba(34,197,94,0.3)":res.biaisH4==="CONTRA"?"rgba(234,179,8,0.3)":res.biaisH4==="RETOURNEMENT"?"rgba(168,85,247,0.3)":"rgba(239,68,68,0.3)"),borderRadius:6,padding:"5px 10px",fontSize:10,color:res.biaisH4==="LONG"?"#22c55e":res.biaisH4==="CONTRA"?"#eab308":res.biaisH4==="RETOURNEMENT"?"#c084fc":"#ef4444",fontWeight:600}}>M30 {res.biaisH4==="LONG"?"▲":res.biaisH4==="CONTRA"?"↕":res.biaisH4==="RETOURNEMENT"?"↩":"▼"} {res.biaisH4}</span>}
...
export default function PivotAgent(){
  ...
  var [activePair,setActivePair]=useState("EURUSD");
  var [newsStatut,setNewsStatut]=useState("VERT");
