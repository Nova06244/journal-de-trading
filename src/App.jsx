...
const SLP_EUR = 0.0010;
const TPP_EUR = 0.0020;
const SLP_GBP = 0.0013;
const TPP_GBP = 0.0026;
const SLP = 0.0010;
const TPP = 0.0020;
...
// ─── CALCUL BIAIS JOURNÉE (règle 2/3) ────────────────────────────────────────
// Retourne { biais: "HAUSSIER"|"BAISSIER", score: 2|3, votes: [...] }
function calcBiaisJournee(biaisM30, biaisM15, biaisM15Ret, gapWeekend) {
  // Résoudre le vote M15 : si retournement → sens opposé à M30
  var voteM30 = biaisM30; // "LONG"|"SHORT"|""
  var voteM15 = biaisM15; // "LONG"|"SHORT"|""
  if (biaisM15Ret && biaisM30) {
    voteM15 = biaisM30 === "LONG" ? "SHORT" : "LONG";
  }
  var voteGap = gapWeekend === "HAUSSIER" ? "LONG" : gapWeekend === "BAISSIER" ? "SHORT" : "";

  var votes = [voteM30, voteM15, voteGap].filter(function(v){ return v !== ""; });
  if (votes.length < 2) return null;

  var longCount = votes.filter(function(v){ return v === "LONG"; }).length;
  var shortCount = votes.filter(function(v){ return v === "SHORT"; }).length;

  if (longCount >= 2) return { biais: "HAUSSIER", score: longCount, total: votes.length };
  if (shortCount >= 2) return { biais: "BAISSIER", score: shortCount, total: votes.length };
  return null;
}
...
// ─── CARTE BIAIS JOURNÉE ─────────────────────────────────────────────────────
function BiaisJourneeCard(props) {
  var bj = props.biais;
  if (!bj) return null;

  var isH = bj.biais === "HAUSSIER";
  var isFort = bj.score === 3;
  var col = isH ? "#22c55e" : "#ef4444";
  var bg = isH ? "rgba(34,197,94,0.08)" : "rgba(239,68,68,0.08)";
  var bd = isH ? "rgba(34,197,94,0.35)" : "rgba(239,68,68,0.35)";
  var icon = isH ? "▲" : "▼";
  var label = isFort ? (isH ? "FORT HAUSSIER" : "FORT BAISSIER") : (isH ? "HAUSSIER" : "BAISSIER");
  var sublabel = bj.score + "/" + bj.total + " critères alignés";

  return (
    <div style={{background:bg,border:"1px solid "+bd,borderRadius:10,padding:14,marginBottom:14}}>
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:10}}>
        <div style={{fontSize:9,color:"#475569",letterSpacing:2}}>BIAIS JOURNÉE</div>
        {isFort && <span style={{fontSize:9,color:col,background:isH?"rgba(34,197,94,0.15)":"rgba(239,68,68,0.15)",borderRadius:4,padding:"2px 8px",fontWeight:700,letterSpacing:1}}>CONFLUENCE FORTE</span>}
      </div>
      <div style={{display:"flex",alignItems:"center",gap:12}}>
        <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:32,color:col,letterSpacing:2,lineHeight:1}}>{icon} {label}</div>
      </div>
      <div style={{marginTop:8,fontSize:10,color:col,opacity:0.7}}>{sublabel}</div>
      <div style={{marginTop:10,display:"flex",gap:6,flexWrap:"wrap"}}>
        {props.votes.map(function(v,i){
          var vc=v.val==="LONG"?"#22c55e":v.val==="SHORT"?"#ef4444":"#475569";
          var vbg=v.val==="LONG"?"rgba(34,197,94,0.1)":v.val==="SHORT"?"rgba(239,68,68,0.1)":"rgba(255,255,255,0.04)";
          return(
            <span key={i} style={{fontSize:9,color:vc,background:vbg,borderRadius:4,padding:"2px 8px",border:"1px solid "+vc+"30"}}>
              {v.label} {v.val==="LONG"?"▲":v.val==="SHORT"?"▼":"—"}
            </span>
          );
        })}
      </div>
...
function TabSignal(props){
  ...
  // ── Calcul biais journée ──────────────────────────────────────────────────
  var biaisJournee = calcBiaisJournee(
    props.biaisM30sel,
    props.biaisM15sel,
    props.isRetournement,
    props.gapWeekend
  );

  // Votes pour affichage dans la carte
  var votesDisplay = [];
  if (props.biaisM30sel) votesDisplay.push({ label:"M30", val: props.biaisM30sel });
  if (props.biaisM15sel || props.isRetournement) {
    var m15val = props.isRetournement && props.biaisM30sel
      ? (props.biaisM30sel === "LONG" ? "SHORT" : "LONG")
      : props.biaisM15sel;
    if (m15val) votesDisplay.push({ label: props.isRetournement ? "M15↩" : "M15", val: m15val });
  }
  if (props.gapWeekend) votesDisplay.push({ label:"GAP", val: props.gapWeekend === "HAUSSIER" ? "LONG" : "SHORT" });
    ...
    <div>
      ...
      <div style={{background:"rgba(255,255,255,0.03)",border:"1px solid rgba(255,255,255,0.06)",borderRadius:10,padding:10,marginBottom:14}}>
        ...
        {props.newsStatut&&props.newsStatut!=="ROUGE"&&(
          <div style={{marginTop:8,fontSize:10,color:"#475569"}}>
            {props.activePair==="EURUSD"
              ?props.newsStatut==="VERT"?"SL 10p · TP 20p · RR 1:2":"SL 10p · TP 15p · RR 1:1.5"
              :props.newsStatut==="VERT"?"SL 13p · TP 26p · RR 1:2":"SL 13p · TP 18p · RR 1:1.5"
            }
          </div>
        )}
      </div>
        ...
        props.newsStatut==="ROUGE"?(
          ...
          <div style={{background:"rgba(34,197,94,0.08)",border:"1px solid rgba(34,197,94,0.3)",borderRadius:10,padding:14,marginBottom:14}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8}}>
              {[{l:"SL",v:props.activePair==="EURUSD"?"10 pips":"13 pips",c:"#ef4444"},{l:"TP",v:props.activePair==="EURUSD"?"20 pips":"26 pips",c:"#22c55e"},{l:"RR",v:"1 : 2",c:"#93c5fd"}].map(function(x){return(
          ...
          <div style={{background:"rgba(234,179,8,0.08)",border:"1px solid rgba(234,179,8,0.3)",borderRadius:10,padding:14,marginBottom:14}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8}}>
              {[{l:"SL",v:props.activePair==="EURUSD"?"10 pips":"13 pips",c:"#ef4444"},{l:"TP",v:props.activePair==="EURUSD"?"15 pips":"18 pips",c:"#eab308"},{l:"RR",v:props.activePair==="EURUSD"?"1 : 1.5":"1 : 1.4",c:"#93c5fd"}].map(function(x){return(
                <div key={x.l} style={{background:"rgba(0,0,0,0.2)",borderRadius:7,padding:"8px 4px",textAlign:"center"}}>
                  <div style={{fontSize:9,color:"#475569",marginBottom:3}}>{x.l}</div>
                  <div style={{fontSize:12,fontWeight:700,color:x.c}}>{x.v}</div>
                </div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 2fr",gap:10,marginBottom:12}}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:8}}>
          ...
          {/* ── BIAIS M15 (avec retournement) ── */}
          <div style={{background:"rgba(255,255,255,0.03)",border:"1px solid rgba(255,255,255,0.06)",borderRadius:10,padding:10}}>
            <div style={{fontSize:9,color:"#475569",marginBottom:6,letterSpacing:1}}>BIAIS M15</div>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:6,marginBottom:6}}>
              <button onClick={function(){props.setBiaisM15sel("");props.setIsRetournement(false);}} style={{padding:"7px 4px",background:props.biaisM15sel===""&&!props.isRetournement?"rgba(255,255,255,0.1)":"rgba(255,255,255,0.03)",border:"1px solid rgba(255,255,255,0.1)",borderRadius:7,color:"#64748b",fontSize:10,cursor:"pointer",fontFamily:"'DM Mono',monospace"}}>—</button>
              <button onClick={function(){props.setBiaisM15sel("LONG");props.setIsRetournement(false);}} style={{padding:"7px 4px",background:props.biaisM15sel==="LONG"&&!props.isRetournement?"rgba(34,197,94,0.2)":"rgba(255,255,255,0.03)",border:"1px solid "+(props.biaisM15sel==="LONG"&&!props.isRetournement?"rgba(34,197,94,0.5)":"rgba(255,255,255,0.1)"),borderRadius:7,color:props.biaisM15sel==="LONG"&&!props.isRetournement?"#22c55e":"#475569",fontSize:10,cursor:"pointer",fontWeight:props.biaisM15sel==="LONG"&&!props.isRetournement?700:400,fontFamily:"'DM Mono',monospace"}}>▲ HAUSSIER</button>
              <button onClick={function(){props.setBiaisM15sel("SHORT");props.setIsRetournement(false);}} style={{padding:"7px 4px",background:props.biaisM15sel==="SHORT"&&!props.isRetournement?"rgba(239,68,68,0.2)":"rgba(255,255,255,0.03)",border:"1px solid "+(props.biaisM15sel==="SHORT"&&!props.isRetournement?"rgba(239,68,68,0.5)":"rgba(255,255,255,0.1)"),borderRadius:7,color:props.biaisM15sel==="SHORT"&&!props.isRetournement?"#ef4444":"#475569",fontSize:10,cursor:"pointer",fontWeight:props.biaisM15sel==="SHORT"&&!props.isRetournement?700:400,fontFamily:"'DM Mono',monospace"}}>▼ BAISSIER</button>
            </div>
            {/* Retournement M15 */}
            {(function(){
              var retDisabled = props.biaisM30sel === "";
              var retCol = props.biaisM30sel==="LONG"?"#ef4444":props.biaisM30sel==="SHORT"?"#22c55e":"#c084fc";
              var retBg = props.biaisM30sel==="LONG"?"rgba(239,68,68,0.2)":props.biaisM30sel==="SHORT"?"rgba(34,197,94,0.2)":"rgba(168,85,247,0.2)";
              var retBd = props.biaisM30sel==="LONG"?"rgba(239,68,68,0.5)":props.biaisM30sel==="SHORT"?"rgba(34,197,94,0.5)":"rgba(168,85,247,0.5)";
              var retLabel = props.biaisM30sel==="LONG"?"↩ RETOURN. → BAISSIER":props.biaisM30sel==="SHORT"?"↩ RETOURN. → HAUSSIER":"↩ RETOURNEMENT";
              return(
                <button
                  onClick={function(){
                    if(!retDisabled){
                      props.setIsRetournement(!props.isRetournement);
                      props.setBiaisM15sel("");
                    }
                  }}
                  disabled={retDisabled}
                  style={{width:"100%",padding:"7px 4px",background:props.isRetournement?retBg:"rgba(255,255,255,0.03)",border:"1px solid "+(props.isRetournement?retBd:"rgba(255,255,255,0.1)"),borderRadius:7,color:props.isRetournement?retCol:retDisabled?"#2d3748":"#475569",fontSize:10,cursor:retDisabled?"not-allowed":"pointer",fontWeight:props.isRetournement?700:400,fontFamily:"'DM Mono',monospace",opacity:retDisabled?0.4:1}}
                >
                  {retLabel}
                </button>
              );
          ...
          <div style={{background:"rgba(255,255,255,0.03)",border:"1px solid rgba(255,255,255,0.06)",borderRadius:10,padding:10}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr 1fr",gap:5}}>
              {[{v:"",l:"—",c:"#64748b",bg:"rgba(255,255,255,0.1)"},{v:"REJET",l:"◆ REJET",c:"#93c5fd",bg:"rgba(59,130,246,0.2)"},{v:"CASS+PB",l:"⤴ CASS+PB",c:"#eab308",bg:"rgba(234,179,8,0.2)"},{v:"SPIKE",l:"↩ SPIKE",c:"#ef4444",bg:"rgba(239,68,68,0.2)"}].map(function(s){
                return(
                  <button key={s.v} onClick={function(){props.setSignalType(s.v);props.setCassPullback(s.v==="CASS+PB");}} style={{padding:"6px 2px",background:props.signalType===s.v?s.bg:"rgba(255,255,255,0.03)",border:"1px solid "+(props.signalType===s.v?s.c+"80":"rgba(255,255,255,0.1)"),borderRadius:7,color:props.signalType===s.v?s.c:"#475569",fontSize:9,cursor:"pointer",fontWeight:props.signalType===s.v?700:400,fontFamily:"'DM Mono',monospace"}}>
                    {s.l}
                  </button>
                );
              })}
      ...
      {/* ── CARTE BIAIS JOURNÉE ── */}
      <BiaisJourneeCard biais={biaisJournee} votes={votesDisplay} />
      ...
      <div style={{marginTop:8,background:"rgba(255,255,255,0.02)",border:"1px solid rgba(255,255,255,0.08)",borderRadius:12,padding:14}}>
        ...
        {/* Résumé contexte */}
        {(props.activePair||props.newsStatut||props.biaisM30sel||props.biaisM15sel||biaisJournee)&&(
          <div style={{background:"rgba(0,0,0,0.2)",borderRadius:8,padding:"8px 10px",marginBottom:10,display:"flex",flexWrap:"wrap",gap:6}}>
            ...
            {props.biaisM30sel&&<span style={{fontSize:9,color:"#94a3b8",background:"rgba(255,255,255,0.05)",borderRadius:4,padding:"2px 6px"}}>{props.biaisM30sel==="LONG"?"▲":"▼"} M30</span>}
            {(props.biaisM15sel||props.isRetournement)&&<span style={{fontSize:9,color:props.isRetournement?"#c084fc":"#94a3b8",background:props.isRetournement?"rgba(168,85,247,0.1)":"rgba(255,255,255,0.05)",borderRadius:4,padding:"2px 6px"}}>{props.isRetournement?"↩ M15":((props.biaisM15sel==="LONG"?"▲":"▼")+" M15")}</span>}
            {biaisJournee&&<span style={{fontSize:9,color:biaisJournee.biais==="HAUSSIER"?"#22c55e":"#ef4444",background:biaisJournee.biais==="HAUSSIER"?"rgba(34,197,94,0.1)":"rgba(239,68,68,0.1)",borderRadius:4,padding:"2px 6px",fontWeight:700}}>{biaisJournee.biais==="HAUSSIER"?"▲":"▼"} {biaisJournee.biais}</span>}
            {props.gapWeekend&&<span style={{fontSize:9,color:"#64748b",background:"rgba(255,255,255,0.05)",borderRadius:4,padding:"2px 6px"}}>GAP {props.gapWeekend==="HAUSSIER"?"▲":"▼"}</span>}
            {props.signalType&&<span style={{fontSize:9,color:"#eab308",background:"rgba(234,179,8,0.1)",borderRadius:4,padding:"2px 6px"}}>{props.signalType}</span>}
          </div>
        )}
        ...
        <div>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
            ...
            <button onClick={function(){
              var headers=["date","heure","pair","niveau","dir","resultat","gain","commission","lot","sl_pips","tp_pips","rr","news_statut","biais_m30","signal_entree","gap_weekend","is_retournement","note"];
    ...
    <div>
      ...
      {newsData&&!newsLoading&&(
        <div>
          ...
          {NewsCard(newsData.fenetre3,"3","RETOURNEMENT 17h-19h")}
...
export default function PivotAgent(){
  ...
  async function onAdd(){
    ...
    var slPips=activePair==="GBPUSD"?13:10;
    var tpPips=activePair==="GBPUSD"?(newsStatut==="ORANGE"?18:26):(newsStatut==="ORANGE"?15:20);
    var rrStr=activePair==="GBPUSD"?(newsStatut==="ORANGE"?"1:1.4":"1:2"):(newsStatut==="ORANGE"?"1:1.5":"1:2");
    // Calcul biais journée au moment de l'enregistrement
    var bj=calcBiaisJournee(biaisM30sel,biaisM15sel,isRetournement,gapWeekend);
    var data={
      ...
      biais_m30:biaisM30sel||null,
      biais_m15:biaisM15sel||(isRetournement&&biaisM30sel?(biaisM30sel==="LONG"?"SHORT":"LONG"):null),
      biais_journee:bj?bj.biais:null,
      signal_entree:signalType||null,
      gap_weekend:gapWeekend||null,
      is_retournement:isRetournement||false
    };
    setLoading(true);
    try {
      if(editTrade){
  ...
  function startEdit(t){
    ...
    if(t.biais_m30)setBiaisM30sel(t.biais_m30);
    if(t.biais_m15)setBiaisM15sel(t.is_retournement?"":t.biais_m15);
    if(t.signal_entree)setSignalType(t.signal_entree);
    if(t.gap_weekend)setGapWeekend(t.gap_weekend);
    if(t.is_retournement)setIsRetournement(t.is_retournement);
    setTab("signal");
    window.scrollTo(0,0);
  }
