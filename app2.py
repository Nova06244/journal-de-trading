...
xaxis=dict(gridcolor=hx(C_BORDER), tickfont=dict(color=hx(C_TEXT), size=13), linecolor=hx(C_BORDER)),
yaxis=dict(gridcolor=hx(C_BORDER), tickfont=dict(color=hx(C_TEXT), size=13), linecolor=hx(C_BORDER)),
showlegend=False,
height=height,
...
with c1:
st.markdown(f”<div class='metric-card'><div class='metric-label'>Trades totaux</div><div class='metric-value' style='color:#00d4ff'>{n}</div></div>”, unsafe_allow_html=True)
with c2:
wr_c = hx(C_GREEN) if wr and wr >= 50 else hx(C_RED) if wr else hx(C_BLUE)
st.markdown(f”<div class='metric-card'><div class='metric-label'>Win Rate</div><div class='metric-value' style='color:{wr_c}'>{wr if wr is not None else 0}%</div><div class='metric-sub'>{wins}W . {losses}L . {bes}BE</div></div>”, unsafe_allow_html=True)
with c3:
...
st.markdown(f”<div class='metric-card'><div class='metric-label'>P&L Net</div><div class='metric-value' style='color:{pl_c}'>{pl_str}</div></div>”, unsafe_allow_html=True)
with c4:
...
st.markdown(f”<div class='metric-card'><div class='metric-label'>Dans le plan</div><div class='metric-value' style='color:{r_c}'>{rp}%</div><div class='metric-sub'>{rules_ok} / {n} trades</div></div>”, unsafe_allow_html=True)
...
with st.form("trade_form", clear_on_submit=not bool(edit_id)):
    ...
    if rr_val:
        ...
        st.markdown(f"<div style='font-size:17px;font-weight:700;color:{rr_col};padding:8px 0;'>Ratio RR : 1 : {rr_val} ({rr_txt})</div>", unsafe_allow_html=True)
...
```
if df.empty:
    st.markdown("<div style='text-align:center;padding:60px 20px;color:#4a7090;font-size:16px;'>Aucun trade. Commence par saisir ton premier trade.</div>", unsafe_allow_html=True)
else:
    ...
    for _, t in df_sorted.iterrows():
        ...
        biais_map = {"bullish":("Haussier",hx(C_GREEN)),"bearish":("Baissier",hx(C_RED)),"neutral":("Indecis",hx(C_YELLOW))}
        biais_label, biais_c = biais_map.get(t["biais"], ("-",hx(C_MUTED)))
        outcome_map = {"Win":("Win",hx(C_GREEN)),"Loss":("Loss",hx(C_RED)),"BE":("BE",hx(C_PURPLE))}
        ...
        with col_card:
            ...
            <div class='trade-card'>
                <div style='display:flex;justify-content:space-between;align-items:center;margin-bottom:10px;'>
                    <div>
                        <span style='color:#00d4ff;font-size:15px;font-weight:600;'>{t["date"]}</span>
                        <span style='color:#6080a0;margin-left:10px;font-size:14px;'>{t["heure"] or ""}</span>
                        <span style='color:{dir_c};margin-left:12px;font-size:15px;font-weight:700;'>{t["direction"]}</span>
                    </div>
                ...
                <div style='font-size:13px;color:#6080a0;'>
                    {f"Entree: {t['entree']}" if t['entree'] else ""}{f" . SL: {t['sl']}" if t['sl'] else ""}{f" . TP: {t['tp']}" if t['tp'] else ""}
                </div>
                {f"<div style='font-size:13px;color:#8090a0;margin-top:8px;border-top:1px solid #1e3040;padding-top:8px;'>{t['raison']}</div>" if pd.notna(t['raison']) and t['raison'] else ""}
                {f"<div style='font-size:13px;color:#00d4ff;margin-top:6px;'>{t['lecon']}</div>" if pd.notna(t['lecon']) and t['lecon'] else ""}
            </div>
            """, unsafe_allow_html=True)
...
```
if df.empty:
    st.markdown("<div style='text-align:center;padding:60px 20px;color:#4a7090;font-size:16px;'>Enregistre des trades pour voir tes statistiques.</div>", unsafe_allow_html=True)
else:
    ...
    if rules_ok < n:
        insight += f" | Plan: {round(rules_ok/n*100)}% - Dans le plan: {'+' if pl_rules>=0 else ''}{pl_rules:.2f}EUR vs Hors plan: {'+' if pl_no_rules>=0 else ''}{pl_no_rules:.2f}EUR"
