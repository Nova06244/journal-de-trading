    ...
    if os.path.exists(SAVE_FILE):
        try:
            ...
            st.session_state["data"] = pd.DataFrame(columns=[
                "Date", "Reward (%)", "Actif", "Résultat", "Session", "Mise (€)", "Risk (%)", "Gain (€)"
            ])
...
def save_data():
    capital_row = pd.DataFrame([{
        "Date": "", "Reward (%)": "", "Actif": "__CAPITAL__", "Résultat": "",
        "Session": "", "Mise (€)": "", "Risk (%)": "", "Gain (€)": st.session_state["capital"]
    }])
...
with col2:
    ...
    if st.button("♻️ Réinitialiser la mise de départ"):
        st.session_state["capital"] = 0.0
        save_data()
        st.success("🔁 Mise de départ réinitialisée")
...
total_breakeven = (df["Résultat"] == "Breakeven").sum()
total_pas_trade = (df["Résultat"] == "Pas de trade").sum()
...
st.download_button("📤 Exporter le journal (CSV)", data=csv, file_name="journal_trading.csv", mime="text/csv")
